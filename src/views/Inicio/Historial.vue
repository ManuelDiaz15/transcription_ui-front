<template>
	<div>
		<v-data-table :headers="headers" :items="Transcripciones"
			item-key="name" class="elevation-1" :search="search"
			:footer-props="{ 'items-per-page-text': elementPerPage, }"> <!-- @click:row="redirecionar" -->
...
export default class Historial extends Vue {
	...
	enviardatos(item){
		this.NombreAudio = item.AudioNombre;
